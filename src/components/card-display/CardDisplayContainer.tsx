...
export function CardDisplayContainer() {
  ...
  // const selectedTab=params.selectedTab as string;
  ...
  useEffect(() => {
    ...
    if (cardId) {
      ...
      if (user && selectedTab !== "local") {
        ...
        if (typeof window !== "undefined") {
          ...
          if (oldStorageCards) {
            try {
              ...
              console.log(
                "Cards in old storage:",
                parsedOld.map((c) => ({
                  id: c.metadata.id,
                  slug: c.metadata.slug,
                }))
              );
              const foundInOld = parsedOld.find(
                (c) => c.metadata.id === cardId
              );
              console.log(
                "Card found in old storage:",
                foundInOld ? "YES" : "NO"
              );
            } catch (e) {
          ...
          if (newStorageCards) {
            try {
              ...
              console.log(
                "Cards in new storage:",
                parsedNew.map((c) => ({
                  id: c.metadata.id,
                  slug: c.metadata.slug,
                }))
              );
              const foundInNew = parsedNew.find(
                (c) => c.metadata.id === cardId
              );
              console.log(
                "Card found in new storage:",
                foundInNew ? "YES" : "NO"
              );
            } catch (e) {
        ...
        console.log(
          "Card found by loadBusinessCards():",
          foundCard ? "YES" : "NO"
        );
